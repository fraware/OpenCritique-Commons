"""Alembic migration path tests (empty DB and previous-release stub)."""
...
def _migrate(tmp_path, monkeypatch, db_name: str) -> str:
    monkeypatch.chdir(tmp_path)
    url = f"sqlite:///{db_name}"
    monkeypatch.setenv("OPENCRITIQUE_DATABASE_URL", url)
    command.upgrade(_alembic_config(), "head")
    return url


def test_migrate_empty_database(tmp_path, monkeypatch) -> None:
    ...
    assert version == "e47498e63a9d"


def test_migrate_from_previous_release_stub(tmp_path, monkeypatch) -> None:
    """First migration: previous-release is empty → current (stubbed)."""
    url = _migrate(tmp_path, monkeypatch, "previous.db")
...
def test_metadata_matches_migrated_schema(tmp_path, monkeypatch) -> None:
